...
const NextAccrual = ({ deposits }: { deposits: IDeposit[] }) => {
  ...
  useEffect(() => {
    if (closestDeposit) {
      const newExpiry = Timestamp.fromMillis(closestDeposit.deposit.lastAccrual._seconds * 1000 + 24 * 60 * 60 * 1000).toDate();
  ...
  return (
    <div className={styles["next-accrual"]}>
      ...
      <div className={styles["timer-wrapper"]}>
        <p className={styles["title"]}>Пусть каждая секунда приближает вас к большему!</p>
