...
const OpenPlanConfirm = () => {
  ...
  return (
    <div className={styles["open-plan-confirm"]}>
      <p><span>План:</span> {PLAN_MAP[selectedPlan]}</p>
      <p><span>Тип плана:</span> {variant.type}</p>
      <p><span>Кошелёк:</span> {selectedWallet.toUpperCase()}</p>
      <p><span>Сумма:</span> ${amount}</p>
      <p><span>Доход в день:</span> ${calculateDailyIncome(amount, variant.inDay)}</p>
      <p><span>Общий доход:</span> ${calculateTotalIncome(amount, variant.inDay, variant.days)}</p>
      <p><span>Дата:</span> 24/09/2024</p>
    </div>
  );
};
